...
public interface TableRepository extends JpaRepository<TableEntity, Long> {
        ...
        // Find available tables for a time slot (tables without conflicting
        // reservations)
        @Query("SELECT t FROM TableEntity t WHERE t.isActive = true " +
                        "AND t.capacity >= :partySize " +
                        "AND t.status NOT IN :excludedTableStatuses " +
                        "AND t.id NOT IN (" +
        ...
        List<TableEntity> findAvailableTables(
                        ...
                        @Param("excludedReservationStatuses") List<ReservationStatus> reservationStatuses,
                        @Param("excludedTableStatuses") List<TableStatus> tableStatuses);
